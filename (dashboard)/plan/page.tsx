...
// ─── Legacy data cleanup ───────────────────────────────────────────────────────
// If stored data is missing plans array OR is older than 24 h → purge and re-prompt.
const LEGACY_KEYS = ["smartstudy_result", "smartstudy_plan", "smartstudy_draft"];
...
function clearLegacyData(): void {
  for (const key of LEGACY_KEYS) {
    ...
    try {
      ...
      const storedAt = typeof parsed._storedAt === "number" ? parsed._storedAt : 0;
      const isExpired = Date.now() - storedAt > MAX_CACHE_AGE_MS;
      // Zombie: no plans and no yearPlan, or timestamp expired
      const isZombie = !Array.isArray(parsed.plans) && !parsed.yearPlan;
      if (isExpired || isZombie) localStorage.removeItem(key);
    } catch {
      localStorage.removeItem(key); // corrupt JSON → nuke it
    }
  }
}
...
export default function PlanPage() {
  ...
  const yearPlanRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    // Purge zombie/expired cache before reading — prevents stale data crash
    clearLegacyData();
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      if (planCardsRef.current) els.push(planCardsRef.current);
      if (yearPlanRef.current) els.push(yearPlanRef.current);
      await downloadAllPdf(els, "smartstudy-plan.pdf");
    } finally {
      setPdfLoading(false);
    }
  };
      ...
      <div className="max-w-[960px] mx-auto">
          ...
          <div className="flex gap-2 shrink-0 items-center flex-wrap">
            ...
            {hasPlans && (
              <button
                ...
                disabled={pdfLoading}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 text-white rounded-full text-xs font-semibold cursor-pointer border-none shadow-[0_4px_10px_rgba(99,102,241,0.3)] transition-colors"
              >
        ...
        {hasPlans && (
          <section className="mb-2">
            ...
            {planView === "card" ? (
              <div ref={planCardsRef} className="flex flex-wrap gap-4">
        ...
        {result.yearPlan && (
          <div ref={yearPlanRef}>
            <YearPlanView yearPlan={result.yearPlan} />
          </div>
        )}
