...
import { buildBlockedTimeSlots, detectOffDayPreference } from "@/lib/planner-logic";
...
이미지의 격자(Grid) 구조를 분석하여 수업 블록 정보와 차단 시간대를 추출하라.
  ...
  "courses": [
    {
      ...
      "endTime": "종료시간 (HH:MM 형식, 없으면 null)",
      "credits": 학점_숫자_또는_null,
      "room":    "강의실 (없으면 null)",
      "gridRow": "격자 행 번호(1-기반, 1=09:00, 2=10:30, 3=12:00, 4=13:30, 5=15:00, 6=16:30)",
...
- blockedTimeSlots: courses의 각 과목 → 요일별로 개별 슬롯 분리
  예) "월수 09:00" 과목 → [{day:"월",startTime:"09:00",endTime:"10:30",...}, {day:"수",...}]
- preferredOffDays: 격자에서 수업 블록이 없는 열(요일)을 모두 포함
- gridRow/gridCol: 이미지 격자에서 블록의 물리적 위치 (1-기반 정수)
...
async function handleVision(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const coursesForBlocking = (parsed.courses ?? []) as Array<{
      name?: string; day?: string; time?: string; code?: string; credits?: number;
    }>;
    const serverBlockedSlots = buildBlockedTimeSlots(
      coursesForBlocking.map((c) => ({
        name: c.name ?? "", day: c.day ?? "", time: c.time ?? "",
        code: c.code ?? undefined, credits: c.credits ?? undefined,
      })),
    );
    ...
    console.info(
      `[vision/BlockedSlots] ${serverBlockedSlots.length}개 슬롯 계산, ` +
      `공강후보: ${detectedOffDay ?? "없음"}`,
    );
