// Proxy route: builds a structured study-plan prompt and forwards to Replit AI backend.
// Includes plan-tier rate-limit skeleton (DB connection required for enforcement).
...
// ─── Demo fallback (returned when upstream is unavailable / 404) ──────────────

const DEMO_PLAN = {
  _isDemo: true,
  planA: {
    title: "학점 극대화 전략",
    strategy: "COR 공통필수를 1학년에 집중 이수하고, LCS 선택과목으로 역량을 다집니다.",
    courses: [
  ...
  planB: {
    title: "진로 집중 전략",
    strategy: "진로 연계 LCS 선택과목과 전공 기초를 우선 배치하여 취업 역량을 강화합니다.",
    courses: [
  ...
  planD: {
    title: "여유 전략",
    strategy: "적정 학점(12학점)을 유지하며 대학 생활 적응과 자기계발에 집중합니다.",
    courses: [
...
/** Extract plan tier and userId from the session cookie (if present). */
async function getRequestor(req: NextRequest): Promise<RequestorInfo> {
  void req; // reserved for future header-based auth
  try {
    const cookieStore = await cookies();
    const raw = cookieStore.get("dh_auth")?.value;
    if (!raw) return { userId: null, plan: "beta" };
    const payload = parseToken(raw);
    if (!payload) return { userId: null, plan: "beta" };
    return { userId: payload.userId, plan: payload.plan };
  } catch {
    return { userId: null, plan: "beta" };
  }
}

/**
 * Check whether the requestor has remaining quota for this month.
 * Returns null if allowed, or a 429 NextResponse if limit is exceeded.
 *
 * TODO: Query Supabase `usage` table for actual monthly count.
 *       Until then this function always returns null (allow all).
 */
async function checkRateLimit(info: RequestorInfo): Promise<NextResponse | null> {
  const limit = PLAN_LIMITS[info.plan];

  // TODO: const usedThisMonth = await db.usage.count({ userId: info.userId, month: currentMonth() });
  const usedThisMonth = 0; // stub — remove once DB is connected

  if (usedThisMonth >= limit) {
    const planLabel = info.plan === "beta" ? "베타(무료)" : info.plan;
    return NextResponse.json(
      {
        error: `이번 달 사용 한도(${limit}회)를 초과했습니다. 더 많은 생성을 원하시면 플랜을 업그레이드해주세요.`,
...
function buildPrompt(studentInfo: string, timetableInfo: string): string {
...
export async function POST(req: NextRequest) {
  try {
    // ── Rate-limit check (plan-tier based) ────────────────────────────────
    const requestor = await getRequestor(req);
    const limitError = await checkRateLimit(requestor);
    if (limitError) return limitError;

    // ── Request body ─────────────────────────────────────────────────────
    const body: StudyPlanRequest = await req.json();
    const { studentInfo, timetableInfo = "", imageUrl } = body;

    if (!studentInfo || typeof studentInfo !== "string") {
      return NextResponse.json(
        { error: "studentInfo 필드가 필요합니다." },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      if (fetchErr instanceof Error && fetchErr.name === "AbortError") {
        // Timeout — serve demo so the user still sees the full plan UI
        return NextResponse.json(DEMO_PLAN);
      }
      throw fetchErr;
    } finally {
      clearTimeout(timer);
    ...
    if (!upstream.ok) {
      // When the AI backend is sleeping / not found, serve the demo plan
      // so users can see the full UI flow without a hard error.
      if (upstream.status === 404 || upstream.status === 503) {
        return NextResponse.json(DEMO_PLAN);
      }
      const errText = await upstream.text();
      return NextResponse.json(
        { error: `업스트림 오류 (${upstream.status}): ${errText.slice(0, 200)}` },
        { status: upstream.status }
      );
    }

    // Try to parse upstream reply as JSON; fall back to raw text.
    ...
    // If Replit wraps result in { reply: "..." }, parse the inner JSON.
    if (
      parsed &&
      ...
      try {
        ...
        return NextResponse.json({ reply: replyStr });
      }
    }
