...
/** Version tag written to localStorage along with plan data.
 * Bump this when the stored data schema changes so stale entries are cleared automatically.
 */
export const RESULT_DATA_VERSION = "v5-rc5";
  ...
  const generate = useCallback(async (input: StudyPlanInput) => {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        localStorage.setItem('smartstudy_last_input', JSON.stringify(enrichedInput));
      }
      router.push('/plan');
    } catch (err: unknown) {
